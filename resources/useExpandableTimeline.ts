...
export function useExpandableTimeline(options: UseExpandableTimelineOptions = {}) {
  const {
    initialMode = 'normal',
    sizes = {},
    onModeChange,
  ...
  // Change mode
  const changeMode = useCallback((newMode: TimelineMode) => {
    setMode(newMode);
  ...
  const cycle = useCallback(() => {
    const modes: TimelineMode[] = ['collapsed', 'normal', 'expanded', 'fullscreen'];
